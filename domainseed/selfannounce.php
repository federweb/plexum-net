...
$NP_SA_DIR   = __DIR__;
$NP_SA_FILES = array(
    'node_identity' => $NP_SA_DIR . '/node_identity.json',
    'node_config'   => $NP_SA_DIR . '/node_config.json',
    'seeds_origin'  => $NP_SA_DIR . '/seeds_origin.json',
    'peers_online'  => $NP_SA_DIR . '/peers_online.json',
    'identity_dir'  => $NP_SA_DIR . '/identity/',
    'last_sa'       => $NP_SA_DIR . '/data/.last_selfannounce',
);
...
function np_maybe_selfannounce() {
    ...
    if (!is_dir($NP_SA_FILES['identity_dir'])) {
        return;
    }

    $config = np_read_json($NP_SA_FILES['node_config']);
    $interval = ($config && isset($config['selfannounce_interval_sec']))
        ? (int)$config['selfannounce_interval_sec']
        : 1800; // 30 minutes default

    $last_file = $NP_SA_FILES['last_sa'];
    $now = time();
...
function np_run_selfannounce() {
    ...
    // Load identity
    $identity = np_read_json($NP_SA_FILES['node_identity']);
    ...
    $privkey_path = $NP_SA_FILES['identity_dir'] . 'private.pem';
    ...
    // Read public key
    $pubkey_path = $NP_SA_FILES['identity_dir'] . 'public.pem';
    ...
    $seeds = np_read_json($NP_SA_FILES['seeds_origin']);
    ...
    $peers = np_read_json($NP_SA_FILES['peers_online']);
